...
class AlgorithmCatalogTest {
    @Test void catalogCoversAllCategoriesAndEveryDefaultRunWorks() {
        var catalog=AlgorithmCatalog.create();
        assertTrue(catalog.size()>=55,"broad algorithm coverage");
        Set<String>categories=catalog.stream().map(AlgorithmDemo::category).collect(Collectors.toSet());
        assertEquals(Set.of("Sorting","Searching & Selection","Strings & Hashing","Graph Algorithms","Trees","Heaps & Advanced Structures","Dynamic Programming & Optimization","Divide & Conquer / Matrix","Algorithm Analysis"),categories);
