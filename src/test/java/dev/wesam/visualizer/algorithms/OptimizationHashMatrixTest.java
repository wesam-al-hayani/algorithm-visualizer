...
class OptimizationHashMatrixTest {
  @Test
  void everyHashStrategyHandlesCollisionsSearchAndDelete() {
    for (EducationalHashTable.Strategy strategy : EducationalHashTable.Strategy.values()) {
      ...
      assertTrue(table.insert(12).success());
      assertTrue(table.search(12).success());
      assertTrue(table.delete(1).success());
      assertFalse(table.search(1).success());
      assertTrue(table.search(12).success());
      assertTrue(table.insert(-3).success());
      table.clear();
      assertFalse(table.search(12).success());
    }
  }

  @Test
  void knapsackDpAndBranchBoundAgree() {
    int[] weights = {2, 3, 4, 5}, values = {3, 4, 5, 8};
    assertEquals(12, OptimizationAlgorithms.knapsack(weights, values, 8).maximumValue());
    assertEquals(
        12, OptimizationAlgorithms.branchAndBoundKnapsack(weights, values, 8).maximumValue());
  }

  @Test
  void tspVariantsAgree() {
  ...
  @Test
  void exactAndApproximationProblems() {
    List<GraphAlgorithms.Edge> triangle =
        List.of(
            new GraphAlgorithms.Edge(0, 1),
            new GraphAlgorithms.Edge(1, 2),
            new GraphAlgorithms.Edge(2, 0));
    ...
    var maxSat =
        OptimizationAlgorithms.exactMaxSat(
            2, List.of(new int[] {1}, new int[] {-1, 2}, new int[] {-2}));
    assertEquals(2, maxSat.satisfiedClauses());
  }

  @Test
  void strassenMatchesOrdinaryIncludingPaddedSize() {
