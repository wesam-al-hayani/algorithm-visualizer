...
class SortSearchStringTest {
  @Test
  void everySortHandlesRepresentativeInputs() {
    ...
    for (SortAlgorithms.Kind kind : SortAlgorithms.Kind.values()) {
      for (int[] input : cases) {
        ...
        for (int i = 1; i < sorted.length; i++)
          assertTrue(sorted[i - 1] <= sorted[i], kind.toString());
  ...
  @Test
  void selectionVariantsFindEveryRankWithDuplicates() {
  ...
  @Test
  void stringAlgorithmsFindOverlappingMatches() {
    for (List<Integer> actual :
        List.of(
