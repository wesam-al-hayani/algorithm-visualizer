...
public final class VisualizationCanvas extends Region {
    ...
    @Override protected double computePrefHeight(double w){return 520;}

    private void redraw() {
        GraphicsContext g=canvas.getGraphicsContext2D();double w=canvas.getWidth(),h=canvas.getHeight();
        g.setFill(BACKGROUND);g.fillRoundRect(0,0,w,h,18,18);
        if(step==null){g.setFill(TEXT);g.setFont(Font.font("System",FontWeight.SEMI_BOLD,20));g.setTextAlign(TextAlignment.CENTER);g.fillText("Choose an algorithm and press Start",w/2,h/2);return;}
        switch(step.kind()){
    ...
    private void drawArray(GraphicsContext g,double w,double h){
        List<Integer>a=step.values();if(a.isEmpty()){drawCentered(g,w,h,"Empty input");return;}
        int max=a.stream().mapToInt(Math::abs).max().orElse(1);double gap=6,left=28,bottom=h-52,usable=w-56,bar=Math.max(3,(usable-gap*(a.size()-1))/a.size());
        for(int i=0;i<a.size();i++){double height=Math.max(5,Math.abs(a.get(i))/(double)Math.max(1,max)*(h-110));double x=left+i*(bar+gap),y=bottom-height;g.setFill(color(i));g.fillRoundRect(x,y,bar,height,8,8);g.setFill(TEXT);g.setFont(Font.font(11));g.setTextAlign(TextAlignment.CENTER);g.fillText(Integer.toString(a.get(i)),x+bar/2,bottom+18);}
    }

    private void drawGraph(GraphicsContext g,double w,double h){
        int n=step.labels().size();if(n==0){drawCentered(g,w,h,"Empty graph");return;}double cx=w/2,cy=h/2,radius=Math.max(70,Math.min(w,h)*.34);double[]x=new double[n],y=new double[n];for(int i=0;i<n;i++){double angle=-Math.PI/2+2*Math.PI*i/n;x[i]=cx+radius*Math.cos(angle);y[i]=cy+radius*Math.sin(angle);}
        g.setLineWidth(2);g.setTextAlign(TextAlignment.CENTER);g.setFont(Font.font(11));
        for(AlgorithmStep.VisualEdge e:step.edges()){g.setStroke(Color.web("#52627b"));g.strokeLine(x[e.from()],y[e.from()],x[e.to()],y[e.to()]);double mx=(x[e.from()]+x[e.to()])/2,my=(y[e.from()]+y[e.to()])/2;g.setFill(Color.web("#a9b7cc"));if(!e.label().isBlank())g.fillText(e.label(),mx,my-5);if(e.directed())drawArrow(g,x[e.from()],y[e.from()],x[e.to()],y[e.to()]);}
        for(int i=0;i<n;i++){g.setFill(color(i));g.fillOval(x[i]-22,y[i]-22,44,44);g.setStroke(Color.web("#dbe7fa"));g.strokeOval(x[i]-22,y[i]-22,44,44);g.setFill(Color.web("#0b1220"));g.setFont(Font.font("System",FontWeight.BOLD,13));g.fillText(step.labels().get(i),x[i],y[i]+5);}
    }

    private void drawArrow(GraphicsContext g,double x1,double y1,double x2,double y2){double a=Math.atan2(y2-y1,x2-x1),ex=x2-25*Math.cos(a),ey=y2-25*Math.sin(a);g.setStroke(Color.web("#52627b"));g.strokeLine(ex,ey,ex-9*Math.cos(a-.55),ey-9*Math.sin(a-.55));g.strokeLine(ex,ey,ex-9*Math.cos(a+.55),ey-9*Math.sin(a+.55));}
