...
public final class AlgorithmVisualizerApp extends Application {
    ...
    private final ToggleButton drawWalls=new ToggleButton("Draw Walls"),setGridStart=new ToggleButton("Set Start"),setGridTarget=new ToggleButton("Set Target");
    private final Button clearPath=new Button("Clear Path"),clearGrid=new Button("Clear Grid"),maze=new Button("Generate Maze");
    private final HBox gridTools=new HBox(8,drawWalls,setGridStart,setGridTarget,clearPath,clearGrid,maze);
    ...
    private Region workspace(){
        ...
        gridTools.setManaged(false);gridTools.setVisible(false);VBox top=new VBox(12,selector,inputBox,gridTools,controls);top.getStyleClass().add("control-card");
    ...
    private void wireActions(){
        ...
        algorithms.valueProperty().addListener((o,old,demo)->{stopPlayback();run=null;frame=0;visualization.show(null);updateButtons(false,false);boolean isGrid=demo!=null&&demo.name().startsWith("Grid ");gridTools.setManaged(isGrid);gridTools.setVisible(isGrid);if(demo!=null){input.setText(demo.defaultInput());inputHint.setText(demo.inputHint());explanation.setText(demo.explanation()+"\n\nTime: "+demo.timeComplexity()+"\nSpace: "+demo.spaceComplexity());pseudocode.setText(demo.pseudocode());operation.setText(demo.name());result.setText("");progress.setText("Step 0 / 0");statistics.getChildren().clear();if(isGrid)previewGrid();}});
        ...
        ToggleGroup editMode=new ToggleGroup();drawWalls.setToggleGroup(editMode);setGridStart.setToggleGroup(editMode);setGridTarget.setToggleGroup(editMode);drawWalls.setSelected(true);
        visualization.setOnGridCellClicked(this::editGridCell);clearPath.setOnAction(e->{reset();previewGrid();});clearGrid.setOnAction(e->{makeClearGrid();previewGrid();});maze.setOnAction(e->{makeMaze();previewGrid();});
    }
    ...
    private void randomize(){AlgorithmDemo demo=algorithms.getValue();if(demo==null)return;Random random=new Random();String name=demo.name();if(demo.category().equals("Sorting")||demo.category().equals("Trees")||demo.category().equals("Heaps & Advanced Structures")){input.setText(randomCsv(random,10));}else if(name.equals("Linear Search")||name.equals("Binary Search")){int[]values=randomValues(random,10);if(name.equals("Binary Search"))java.util.Arrays.sort(values);input.setText(csv(values)+" ; "+values[random.nextInt(values.length)]);}else if(name.equals("Quickselect")||name.equals("Median of Medians")){input.setText(randomCsv(random,11)+" ; "+random.nextInt(11));}else if(demo.category().equals("Strings & Hashing")&&name.contains("Probing")||name.equals("Separate Chaining")||name.equals("Double Hashing")){input.setText(randomCsv(random,9)+",?"+(random.nextInt(70)+1));}else if(name.contains("Knapsack")){input.setText("2,3,4,5,6 ; 3,5,6,8,9 ; 12");}else if(name.startsWith("Grid ")){makeMaze();previewGrid();return;}else input.setText(demo.defaultInput());reset();}
    private int[] randomValues(Random random,int count){int[]values=new int[count];for(int i=0;i<count;i++)values[i]=random.nextInt(91)-20;return values;}
    private String randomCsv(Random random,int count){return csv(randomValues(random,count));}
    private String csv(int[]values){return Arrays.toString(values).replace("[","").replace("]","");}
    private void previewGrid(){try{AlgorithmRun preview=algorithms.getValue().runner().apply(input.getText());if(!preview.steps().isEmpty())visualization.show(preview.steps().get(0));}catch(Exception ignored){}}
    private void editGridCell(int row,int column){if(!gridTools.isVisible())return;String[]rows=input.getText().split("/");if(row>=rows.length||column>=rows[row].length())return;char[][]grid=new char[rows.length][];for(int i=0;i<rows.length;i++)grid[i]=rows[i].toCharArray();if(setGridStart.isSelected()){replace(grid,'S','.');if(grid[row][column]!='T')grid[row][column]='S';}else if(setGridTarget.isSelected()){replace(grid,'T','.');if(grid[row][column]!='S')grid[row][column]='T';}else if(grid[row][column]!='S'&&grid[row][column]!='T')grid[row][column]=grid[row][column]=='#'?'.':'#';input.setText(joinGrid(grid));reset();previewGrid();}
    private void makeClearGrid(){String[]rows=input.getText().split("/");int r=Math.max(2,rows.length),c=Math.max(2,rows[0].length());char[][]grid=new char[r][c];for(char[]line:grid)java.util.Arrays.fill(line,'.');grid[0][0]='S';grid[r-1][c-1]='T';input.setText(joinGrid(grid));reset();}
    private void makeMaze(){String[]rows=input.getText().split("/");int r=Math.max(5,rows.length),c=Math.max(6,rows[0].length());char[][]grid=new char[r][c];Random random=new Random();for(int i=0;i<r;i++)for(int j=0;j<c;j++)grid[i][j]=random.nextDouble()<.27?'#':'.';for(int i=0;i<r;i++)grid[i][0]='.';for(int j=0;j<c;j++)grid[r-1][j]='.';grid[0][0]='S';grid[r-1][c-1]='T';input.setText(joinGrid(grid));reset();}
    private void replace(char[][]grid,char target,char replacement){for(char[]row:grid)for(int i=0;i<row.length;i++)if(row[i]==target)row[i]=replacement;}
    private String joinGrid(char[][]grid){StringBuilder out=new StringBuilder();for(int i=0;i<grid.length;i++){if(i>0)out.append('/');out.append(grid[i]);}return out.toString();}
