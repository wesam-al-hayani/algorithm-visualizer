...
public final class GraphAlgorithms {
    ...
    public record Edge(int from, int to, int weight) {
        public Edge(int from, int to) { this(from, to, 1); }
    }
    ...
    public record Graph(int vertices, List<Edge> edges, boolean directed) {
        public Graph {
            ...
            for (Edge e : edges) if (e.from < 0 || e.to < 0 || e.from >= vertices || e.to >= vertices)
    ...
    public record ShortestPaths(long[] distance, int[] parent, boolean negativeCycle) {
        public ShortestPaths { distance = distance.clone(); parent = parent.clone(); }
    ...
    public record MstResult(List<Edge> edges, long totalWeight, boolean spanning) {
        public MstResult { edges = List.copyOf(edges); }
    }
    ...
    public static List<Integer> bfs(Graph graph, int start) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (Edge edge : adj.get(current)) if (!seen[edge.to]) { seen[edge.to] = true; queue.add(edge.to); }
        }
        return order;
    }
    ...
    private static void dfsVisit(int current, List<List<Edge>> adj, boolean[] seen, List<Integer> order) {
        seen[current] = true; order.add(current);
    ...
    public static List<Set<Integer>> connectedComponents(Graph graph) {
        if (graph.directed) throw new IllegalArgumentException("connected components require an undirected graph");
        boolean[] seen = new boolean[graph.vertices];
        List<List<Edge>> adj = graph.adjacency();
        List<Set<Integer>> result = new ArrayList<>();
        for (int start = 0; start < graph.vertices; start++) if (!seen[start]) {
            Set<Integer> component = new LinkedHashSet<>();
            Queue<Integer> queue = new ArrayDeque<>(); queue.add(start); seen[start] = true;
            while (!queue.isEmpty()) {
                int current = queue.remove(); component.add(current);
                for (Edge e : adj.get(current)) if (!seen[e.to]) { seen[e.to] = true; queue.add(e.to); }
            }
    ...
    public static List<Integer> topologicalSort(Graph graph) {
        if (!graph.directed) throw new IllegalArgumentException("topological sort requires a directed graph");
        ...
        List<List<Edge>> adj = graph.adjacency();
        while (!queue.isEmpty()) {
            int current = queue.remove(); result.add(current);
            for (Edge e : adj.get(current)) if (--indegree[e.to] == 0) queue.add(e.to);
        }
        if (result.size() != graph.vertices) throw new IllegalArgumentException("graph contains a directed cycle");
        return result;
    }
    ...
    public static List<Set<Integer>> stronglyConnectedComponents(Graph graph) {
        ...
        List<Edge> reversedEdges = graph.edges.stream().map(e -> new Edge(e.to, e.from, e.weight)).toList();
        List<List<Edge>> reverse = new Graph(graph.vertices, reversedEdges, true).adjacency();
        Arrays.fill(seen, false);
        List<Set<Integer>> components = new ArrayList<>();
        while (!finish.isEmpty()) {
            int v = finish.pop();
            if (!seen[v]) { Set<Integer> part = new LinkedHashSet<>(); collect(v, reverse, seen, part); components.add(part); }
        }
        return components;
    }

    private static void finishDfs(int v, List<List<Edge>> adj, boolean[] seen, Deque<Integer> finish) {
        seen[v] = true;
    ...
    public static ShortestPaths dijkstra(Graph graph, int source) {
        ...
        for (Edge e : graph.edges) if (e.weight < 0) throw new IllegalArgumentException("Dijkstra requires non-negative weights");
        long[] distance = new long[graph.vertices]; Arrays.fill(distance, INF); distance[source] = 0;
        int[] parent = new int[graph.vertices]; Arrays.fill(parent, -1);
        boolean[] settled = new boolean[graph.vertices];
        PriorityQueue<NodeDistance> queue = new PriorityQueue<>(Comparator.comparingLong(NodeDistance::distance));
        queue.add(new NodeDistance(source, 0));
        List<List<Edge>> adj = graph.adjacency();
        while (!queue.isEmpty()) {
            NodeDistance item = queue.remove(); int current = item.node;
            if (settled[current]) continue;
            settled[current] = true;
            for (Edge e : adj.get(current)) if (distance[e.to] > distance[current] + e.weight) {
                distance[e.to] = distance[current] + e.weight; parent[e.to] = current;
    ...
    public static ShortestPaths bellmanFord(Graph graph, int source) {
        ...
        for (int pass = 1; pass < graph.vertices; pass++) {
            ...
            for (Edge e : edges) if (distance[e.from] < INF && distance[e.to] > distance[e.from] + e.weight) {
                distance[e.to] = distance[e.from] + e.weight; parent[e.to] = e.from; changed = true;
            }
            if (!changed) break;
        }
        boolean negativeCycle = false;
        for (Edge e : edges) if (distance[e.from] < INF && distance[e.to] > distance[e.from] + e.weight) negativeCycle = true;
    ...
    public static MstResult kruskal(Graph graph) {
        ...
        List<Edge> chosen = new ArrayList<>(); long total = 0;
        for (Edge e : sorted) if (sets.union(e.from, e.to)) {
            chosen.add(e); total += e.weight;
    ...
    public static MstResult prim(Graph graph, int start) {
        requireUndirected(graph); checkVertex(graph, start);
        boolean[] inTree = new boolean[graph.vertices]; inTree[start] = true;
        ...
        while (!candidates.isEmpty() && chosen.size() < graph.vertices - 1) {
            ...
            inTree[edge.to] = true; chosen.add(edge); total += edge.weight;
    ...
    public static int maximumBipartiteMatching(boolean[][] edges) {
        ...
        for (boolean[] row : edges) if (row.length != rightSize) throw new IllegalArgumentException("ragged matrix");
        int[] rightMatch = new int[rightSize]; Arrays.fill(rightMatch, -1);
        int matches = 0;
    ...
    private static boolean augment(int left, boolean[][] edges, boolean[] seen, int[] rightMatch) {
        for (int right = 0; right < edges[left].length; right++) if (edges[left][right] && !seen[right]) {
            seen[right] = true;
            if (rightMatch[right] == -1 || augment(rightMatch[right], edges, seen, rightMatch)) {
                rightMatch[right] = left; return true;
            }
        }
        return false;
    }
    ...
    public static FlowResult edmondsKarp(int[][] capacity, int source, int sink) {
        ...
        for (int i = 0; i < n; i++) {
            if (capacity[i].length != n) throw new IllegalArgumentException("capacity matrix must be square");
            for (int j = 0; j < n; j++) {
                if (capacity[i][j] < 0) throw new IllegalArgumentException("capacity cannot be negative");
                residual[i][j] = capacity[i][j];
            }
        }
        int maxFlow = 0; int[] parent = new int[n];
        while (flowBfs(residual, source, sink, parent)) {
            int bottleneck = Integer.MAX_VALUE;
            for (int v = sink; v != source; v = parent[v]) bottleneck = Math.min(bottleneck, residual[parent[v]][v]);
            for (int v = sink; v != source; v = parent[v]) {
                int u = parent[v]; residual[u][v] -= bottleneck; residual[v][u] += bottleneck;
            }
            maxFlow += bottleneck;
        }
        boolean[] sourceSide = new boolean[n]; Queue<Integer> queue = new ArrayDeque<>();
        queue.add(source); sourceSide[source] = true;
        while (!queue.isEmpty()) { int u = queue.remove(); for (int v = 0; v < n; v++) if (!sourceSide[v] && residual[u][v] > 0) { sourceSide[v] = true; queue.add(v); } }
    ...
    private static boolean flowBfs(int[][] residual, int source, int sink, int[] parent) {
        ...
        Queue<Integer> queue = new ArrayDeque<>(); queue.add(source);
        while (!queue.isEmpty()) {
            int u = queue.remove();
            for (int v = 0; v < residual.length; v++) if (parent[v] == -1 && residual[u][v] > 0) {
                parent[v] = u; if (v == sink) return true; queue.add(v);
            }
        }
        return false;
    }
    ...
    public record FlowResult(int maximumFlow, int[][] residual, boolean[] sourceSideOfMinCut) {
        public FlowResult {
            ...
            residual = copy; sourceSideOfMinCut = sourceSideOfMinCut.clone();
        }
    }
    ...
    private static List<Edge> expandedEdges(Graph graph) {
        ...
        for (Edge e : graph.edges) { result.add(e); result.add(new Edge(e.to, e.from, e.weight)); }
        return result;
    }
    private static void checkVertex(Graph graph, int vertex) {
        if (vertex < 0 || vertex >= graph.vertices) throw new IllegalArgumentException("vertex outside graph");
    }
    private static void requireUndirected(Graph graph) {
        if (graph.directed) throw new IllegalArgumentException("minimum spanning trees require an undirected graph");
    }
}
