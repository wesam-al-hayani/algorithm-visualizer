...
public final class EducationalHashTable {
  ...
  public record OperationResult(
      boolean success, int hash, List<Integer> probes, int collisions, String message) {
  ...
  public OperationResult insert(int key) {
    ...
    if (chains != null) {
      probes.add(hash);
      List<Integer> bucket = chains.get(hash);
      if (bucket.contains(key))
        return result(false, hash, probes, collisions, "Key already present");
    ...
    for (int attempt = 0; attempt < slots.length; attempt++) {
      ...
      if (slots[index] == null) {
        ...
        return result(
            ...
            probes,
            collisions,
            "Inserted at slot " + (firstTombstone >= 0 ? firstTombstone : index));
      }
      if (slots[index].equals(key))
        return result(false, hash, probes, collisions, "Key already present");
      if (slots[index].equals(TOMBSTONE)) {
        if (firstTombstone < 0) firstTombstone = index;
      } else collisions++;
    }
    if (firstTombstone >= 0) {
      slots[firstTombstone] = key;
      return result(true, hash, probes, collisions, "Reused deleted slot");
    }
  ...
  public OperationResult search(int key) {
    ...
    if (chains != null) {
      probes.add(hash);
      boolean found = chains.get(hash).contains(key);
      return result(
          found,
          hash,
          probes,
          Math.max(0, chains.get(hash).size() - (found ? 1 : 0)),
          found ? "Key found" : "Key not found");
    }
    for (int attempt = 0; attempt < slots.length; attempt++) {
      int index = probe(key, attempt);
      probes.add(index);
      ...
      if (slots[index].equals(key))
        return result(true, hash, probes, collisions, "Key found at slot " + index);
  ...
  public OperationResult delete(int key) {
    ...
    if (!search.success)
      return new OperationResult(
          false, search.hash, search.probes, search.collisions, "Key not found");
    if (chains != null) chains.get(search.hash).remove(Integer.valueOf(key));
    else {
      for (int index : search.probes)
  ...
  public int capacity() {
    return chains != null ? chains.size() : slots.length;
  }
  ...
  public List<String> snapshot() {
    ...
    if (chains != null) for (List<Integer> chain : chains) result.add(chain.toString());
      ...
      for (Integer slot : slots)
        result.add(slot == null ? "—" : slot.equals(TOMBSTONE) ? "×" : slot.toString());
  ...
  private void validateKey(int key) {
    if (key == TOMBSTONE) throw new IllegalArgumentException("reserved key");
  }
}
