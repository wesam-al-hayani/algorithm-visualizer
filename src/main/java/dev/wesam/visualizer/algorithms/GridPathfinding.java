...
public final class GridPathfinding {
  ...
  public static Result find(boolean[][] walls, Cell start, Cell target, Method method) {
    ...
    if (!valid(start, rows, columns) || !valid(target, rows, columns))
      throw new IllegalArgumentException("invalid endpoint");
    for (boolean[] row : walls)
      if (row.length != columns) throw new IllegalArgumentException("ragged grid");
    if (walls[start.row][start.column] || walls[target.row][target.column])
      return new Result(List.of(), List.of(), -1);
    int total = rows * columns, source = id(start, columns), goal = id(target, columns);
    int[] parent = new int[total], distance = new int[total];
    ...
    if (method == Method.BFS || method == Method.DFS) {
      ...
      while (!frontier.isEmpty()) {
        ...
        visited.add(cell(current, columns));
        if (current == goal) break;
        for (int next : neighbors(current, walls, rows, columns))
          if (!done[next] && parent[next] == -1) {
            parent[next] = current;
  ...
  private static List<Integer> neighbors(int id, boolean[][] walls, int rows, int columns) {
    Cell cell = cell(id, columns);
    List<Integer> result = new ArrayList<>(4);
    for (int[] d : DIRECTIONS) {
      int r = cell.row + d[0], c = cell.column + d[1];
      if (r >= 0 && c >= 0 && r < rows && c < columns && !walls[r][c]) result.add(r * columns + c);
    }
    return result;
  }
  ...
  private static boolean valid(Cell cell, int rows, int columns) {
    return cell.row >= 0 && cell.column >= 0 && cell.row < rows && cell.column < columns;
  }

  private static int id(Cell cell, int columns) {
    return cell.row * columns + cell.column;
  }

  private static Cell cell(int id, int columns) {
    return new Cell(id / columns, id % columns);
  }
