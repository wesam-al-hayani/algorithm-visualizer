...
public final class StringAlgorithms {
    ...
    public static List<Integer> naive(String text, String pattern) {
        ...
        if (pattern.isEmpty()) { for (int i = 0; i <= text.length(); i++) matches.add(i); return matches; }
    ...
    public static List<Integer> rabinKarp(String text, String pattern) {
        ...
        for (int start = 0; start + pattern.length() <= text.length(); start++) {
            if (patternHash == windowHash && text.regionMatches(start, pattern, 0, pattern.length())) matches.add(start);
