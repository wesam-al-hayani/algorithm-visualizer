...
public final class BinomialHeap {
  ...
  public Node insert(int key) {
    BinomialHeap one = new BinomialHeap();
  ...
  public int findMinimum() {
    ...
    for (Node n = head.sibling; n != null; n = n.sibling) minimum = Math.min(minimum, n.key);
    return minimum;
  }
  ...
  public int extractMinimum() {
    ...
    for (Node current = head; current != null; current = current.sibling) {
    ...
    if (minPrevious == null) head = min.sibling;
    else minPrevious.sibling = min.sibling;
    Node child = min.child, reversed = null;
    int childCount = (1 << min.degree) - 1;
    while (child != null) {
  ...
  public List<Integer> rootDegrees() {
    List<Integer> out = new ArrayList<>();
    for (Node n = head; n != null; n = n.sibling) out.add(n.degree);
    return out;
  }

  public boolean invariantHolds() {
    int previousDegree = -1, count = 0;
    for (Node n = head; n != null; n = n.sibling) {
      if (n.degree <= previousDegree) return false;
      previousDegree = n.degree;
      int nodes = validate(n, null);
  ...
  private int validate(Node n, Node parent) {
    if (n.parent != parent) return -1;
    int children = 0, count = 1, expected = n.degree - 1;
    for (Node c = n.child; c != null; c = c.sibling) {
      if (c.key < n.key || c.degree != expected--) return -1;
      int sub = validate(c, n);
      if (sub < 0) return -1;
      count += sub;
      children++;
    }
    return children == n.degree ? count : -1;
  }
