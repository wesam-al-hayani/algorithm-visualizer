...
public final class BinarySearchTree {
    ...
    public boolean insert(int key) {
        ...
        while (true) {
            ...
            if (key < current.key) {
                if (current.left == null) { current.left = new Node(key); return true; }
                current = current.left;
            } else {
                if (current.right == null) { current.right = new Node(key); return true; }
    ...
    public boolean delete(int key) {
        ...
        root = delete(root, key); return true;
    }
    ...
    public List<Integer> levelOrder() {
        List<Integer> out = new ArrayList<>(); if (root == null) return out;
        Queue<Node> queue = new ArrayDeque<>(); queue.add(root);
        while (!queue.isEmpty()) { Node n = queue.remove(); out.add(n.key); if (n.left != null) queue.add(n.left); if (n.right != null) queue.add(n.right); }
        return out;
    }
    private void inorder(Node n,List<Integer> out){if(n!=null){inorder(n.left,out);out.add(n.key);inorder(n.right,out);}}
    private void preorder(Node n,List<Integer> out){if(n!=null){out.add(n.key);preorder(n.left,out);preorder(n.right,out);}}
    private void postorder(Node n,List<Integer> out){if(n!=null){postorder(n.left,out);postorder(n.right,out);out.add(n.key);}}
}
